...
VeViMdeltaf::VeViMdeltaf(const Block_id &block_id, Mpi_boundary &mpi_boundary, const Parameter &parameter)
...
int VeViMdeltaf::get_scheme_id() { return parameter_.kVeViMdeltaf; }


real VeViMdeltaf::get_dt(int species) {
...
int VeViMdeltaf::get_data_size() {
  ...
  return parameter_.species[parameter_.kElectron].ncells_xv +
         ...
         29*parameter_.ncells_x + 2;
}


void VeViMdeltaf::init() {
  ...
  if (parameter_.restart) {
    ...
    restart::load_field(maxwell_->E, 4, parameter_.output_directory, block_id_.my_coords);
    restart::load_field(maxwell_->B, 5, parameter_.output_directory, block_id_.my_coords);
    restart::load_field(maxwell_->B_ipol, 6, parameter_.output_directory, block_id_.my_coords);
  ...
  for (int d = 0; d < parameter_.kNDirections; ++d) {
    if (block_id_.neighbour_scheme_ids[d] == parameter_.kVeViMdeltaf) {
...
void VeViMdeltaf::step() {
  ...
  // fluid rk step 1
  fluid_e_->calc_single_step_rk3(ten_moments_tmp_e_, total_flux_e_, source_e_, Q_raw_old_e_, E_cc_, B_cc_);
  ...
  // fluid rk step 2
  fluid_e_->calc_single_step_rk3(ten_moments_tmp_e_, total_flux_e_, source_e_, Q_raw_new_e_, E_cc_, B_cc_);
  ...
  if (parameter_.cweno_calc_j_from_flux) {
    fluid_e_->calc_un_from_flux(un_e_, true);
    if (ion_step_) {
      fluid_i_->calc_un_from_flux(un_i_, true);
    }
...
void VeViMdeltaf::exchange_plasma_solvers(real* data_vlasov_e, real* data_vlasov_i,
  ...
  for (int d = 0; d < parameter_.kNDirections; ++d) {
    ...
    if (block_id_.neighbour_scheme_ids[d] == parameter_.kF10eViMdeltaf && !runge_kutta_only) {
      // update f from received ten moments
      ...
      convert::ten_moments_to_f(buffer_tmp, buffer_in_e[d],
              ...
              parameter_.species[parameter_.kElectron], parameter_);
      boundary_.apply_buffer(data_vlasov_e, buffer_tmp, d, 1, true, parameter_.kElectron);
      #pragma acc exit data delete(buffer_tmp[0:buffer_size_tmp])
      delete[] buffer_tmp;
      if (parameter_.ten_moment_vlasov_coupling_fit) {
        convert::delta_f_fit(data_vlasov_e, buffer_in_e[d], boundary_.calc_buffer_array_dimensions(d),
...
void VeViMdeltaf::send_model_data(real* out_buffer, int receiver_id) {
  
  model::Model *plasma_models[4] = {vlasov_e_, vlasov_i_, fluid_e_, fluid_i_};
  load_balancing::send_model_data(out_buffer, get_data_size(), plasma_models, maxwell_, 4,
                                  receiver_id, boundary_, parameter_);
}


void VeViMdeltaf::receive_model_data(real* in_buffer, int sender_id) {

  model::Model *plasma_models[4] = {vlasov_e_, vlasov_i_, fluid_e_, fluid_i_};
  load_balancing::receive_model_data(in_buffer, get_data_size(), plasma_models, maxwell_, 4,
...
void VeViMdeltaf::replace_dealloc_old_models(model::Model* plasma_model[],
...
void VeViMdeltaf::calc_alloc_converted_model(model::Model*& model_out,
                  int species, int target_scheme_id) {
  // convert models when the scheme is changed

  switch (species) {
    case Parameter::kElectron: {
      if (target_scheme_id == parameter_.kF10eViMdeltaf || target_scheme_id == parameter_.kF10eF10iM) { // to 10 mom
    ...
    case Parameter::kIon: {
      if (target_scheme_id == parameter_.kF10eViMdeltaf) { // to f
...
void VeViMdeltaf::output(real t, int output_number, bool output_vtk) {
  model::Model *plasma_models[4] = {vlasov_e_, vlasov_i_,fluid_e_,fluid_i_};
  output::prepare_and_write(plasma_models, maxwell_, 4, t, output_number, output_vtk,
                            block_id_, parameter_, electron_substep_counter_);
}


int VeViMdeltaf::evaluate_criterion() {
