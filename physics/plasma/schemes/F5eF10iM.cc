...
// five moments fluid electrons, ten moments fluid ions, Maxwell

F5eF10iM::F5eF10iM(const Block_id &block_id, Mpi_boundary &mpi_boundary, const Parameter &parameter)
...
int F5eF10iM::get_scheme_id() { return parameter_.kF5eF10iM; }


real F5eF10iM::get_dt(int species) {
...
int F5eF10iM::get_data_size() {
  // five_moments_e (5*ncells), ten_moments_i (10*ncells)
  // E (3*ncells), B (3*ncells), B_ipol (3*ncells), maxwell_dt (1), maxwell_subcycles (1)
  return 24*parameter_.ncells_x + 2;
}
...
void F5eF10iM::init() {
  ...
  if (parameter_.restart) {
    ...
    convert::un_to_j(j, &fluid_e_->five_moments[parameter_.ncells_x],
                     ...
                     parameter_); // five_/ten_moments(1:3) = n u
...
void F5eF10iM::step() {
  ...
  if (ion_step_) {
    ...
    heat_flux_closure(fluid_i_, ten_moments_tmp_i_, source_i_);
    fluid_i_->apply_single_step_rk3(dt_*electron_substeps_, ten_moments_tmp_i_, total_flux_i_, source_i_, 1., 0., 1.);
  }
  exchange_plasma_solvers(five_moments_tmp_e_, ten_moments_tmp_i_);
...
void F5eF10iM::heat_flux_closure(model::Fluid10 *fluid, const real *ten_moments_tmp, real *source) {
  if (parameter_.ten_moment_closure == "gradient" ||
    ...
    for (int i = 0; i < parameter_.gradient_closure_subcycles; ++i) { // subcycling of laplace P
      fluid->gradient_heat_flux_closure_cycle(i, dt_*electron_substeps_, ten_moments_tmp, source, P_m_tmp);
...
void F5eF10iM::exchange_plasma_solvers(real* data_e, real* data_i) {
  ...
  std::string orientation_five[5] = {"ccc","ccc","ccc","ccc","ccc"};
  std::string orientation_ten[10] = {"ccc","ccc","ccc","ccc","ccc","ccc","ccc","ccc","ccc","ccc"};
  ...
  for (int d = 0; d < parameter_.kNDirections; ++d) {
    ...
    if (block_id_.neighbour_scheme_ids[d] == parameter_.kF10eF10iM ||
    ...
    else if (block_id_.neighbour_scheme_ids[d] == parameter_.kF5eF5iM) {
      // electrons
      exchange_this_direction_e[d] = true;
      buffer_size_e[d] = boundary_.calc_buffer_size(d, 5);
      buffer_out_e[d] = new real[buffer_size_e[d]];
      ...
      if (ion_step_) {
        ...
        boundary_.extract_buffer(data_i, buffer_tmp, d, 10);
        convert::ten_moments_to_five_moments(buffer_out_i[d], buffer_tmp,
  ...
  for (int d = 0; d < parameter_.kNDirections; ++d) {
    ...
    if (block_id_.neighbour_scheme_ids[d] == parameter_.kF10eF10iM ||
      ...
      if (ion_step_) {
        ...
        real* buffer_tmp = new real[buffer_size_tmp];
        #pragma acc enter data create(buffer_tmp[buffer_size_tmp])
        convert::five_moments_to_ten_moments(buffer_tmp, buffer_in_i[d],
                boundary_.calc_buffer_array_dimensions(d), parameter_);
        boundary_.apply_buffer(data_i, buffer_tmp, d, 10);
        #pragma acc exit data delete(buffer_tmp[buffer_size_tmp])
        delete[] buffer_tmp;
        convert::five_ten_moments_fit(data_i, parameter_, d);
      }
    }
    ...
    boundary_.apply_boundary_conditions(data_e, parameter_.bd_cond_five_moments,
            orientation_five, d, 5);
    if (ion_step_) {
      boundary_.apply_boundary_conditions(data_i, parameter_.bd_cond_ten_moments,
              orientation_ten, d, 10);
    }
  }
}


void F5eF10iM::send_model_data(real* out_buffer, int receiver_id) {
...
void F5eF10iM::receive_model_data(real* in_buffer, int sender_id) {
...
void F5eF10iM::replace_dealloc_old_models(model::Model* plasma_model[],
...
void F5eF10iM::calc_alloc_converted_model(model::Model*& model_out,
...
void F5eF10iM::output(real t, int output_number, bool output_vtk) {
  model::Model *plasma_models[2] = {fluid_e_, fluid_i_};
...
int F5eF10iM::evaluate_criterion() {
  model::Model *plasma_models[2] = {fluid_e_, fluid_i_};
